...
// ==================================================================
    ...
    var xhr,
        msg_strings = gf_admin_global_var.box_messages,
        msg_icons = gf_admin_global_var.box_icons;
...
// ===================================================================
    ...
    var infobox = '#gf-infobox',
        infobox_msg = '.gf-infobox-msg',
        infobox_loading = 'gf-infobox-loading';

    $(document).on('click', infobox, function (e) {
        if ($(infobox).is(':visible') && ($(e.target).is('.gf-info-inner') || $(e.target).is('.gf-close-infobox'))) {
    ...
    function show_message_error(ajax_data, response) {
        $(infobox_msg).html(gf_admin_global_var.box_icons.error + response.message);
        setTimeout(function () {
            $(infobox).removeClass(infobox_loading);
        }, 1500);
    }
...
// ==================================================================
    ...
    $(document).on('click', '#gf_settings_save, #gf_settings_reset', function () {

        var func = $(this).data('action'),
            reset = (func === 'gf_reset_settings') ? true : null,
            setting_data = get_settings_data(reset);

        Ajax_Helper({
            nonce: gf_admin_global_var.nonce,
            action: 'gf_admin_ajax',
            func: func,
            reset: reset,
    ...
    function show_save_settings_success(ajax_data, response, msg) {
        ...
        setTimeout(function () {
            $(infobox).removeClass(infobox_loading);
            if (response.content) {
                $('.gf-settings').html($(response.content).html());
                GF_JS.init();
            }
        }, 800);
    }
